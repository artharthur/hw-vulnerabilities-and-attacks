...
- **SYN-скан**: посылается только SYN. Если порт открыт → SYN/ACK, закрыт → RST. Соединение не завершается полностью.  
...
- **UDP-скан**: если порт закрыт → ICMP Port Unreachable; если открыт → нет ответа или специфический UDP-ответ.
...
В Wireshark видно, что TCP-сканы отличаются по флагам пакетов: SYN — «полуоткрытый», FIN и Xmas используют необычные комбинации. UDP-скан выделяется тем, что работает без установки соединения. 
